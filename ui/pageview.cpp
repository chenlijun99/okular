...
/*		Enrico and me have decided this is not worth the trouble it generates
        ...
        case Okular::Settings::EnumMouseMode::RectSelect:
        {
            ...
            } pdfViewRect;
            {
              ...
              auto zoomedPxToPt = [this](int px) {
                return (px / this->d->zoomFactor) * 72 / this->physicalDpiX();
              };
              auto findSingleBoundingPage = [&selectionRect,
                                             this]() -> PageViewItem * {
                PageViewItem *found_item = nullptr;
                foreach (const auto &item, this->d->visibleItems) {
              ...
              if (selectionRectBoundingPage) {
                {
                  const auto &pageGeometry =
                      selectionRectBoundingPage->uncroppedGeometry();
                  pdfViewRect.ptsX =
                      zoomedPxToPt(selectionRect.left() - pageGeometry.left());
                  pdfViewRect.ptsY =
                      zoomedPxToPt(selectionRect.top() - pageGeometry.top());
                  pdfViewRect.ptsWidth = zoomedPxToPt(selectionRect.width());
                  pdfViewRect.ptsHeight = zoomedPxToPt(selectionRect.height());
                }

                menu.addAction(new OKMenuTitle(
                    &menu, i18n("ViewRect coordinates (x:%1, y:%2, w:%3, h:%4 pt)",
                                pdfViewRect.ptsX, pdfViewRect.ptsY,
                                pdfViewRect.ptsWidth, pdfViewRect.ptsHeight)));

                coordinatesToClipboard = menu.addAction(
            ...
            if (choice == coordinatesToClipboard) {
                QClipboard *cb = QApplication::clipboard();
                QString coordinatesText =
                    QString("%1,%2,%3,%4")
                        .arg(pdfViewRect.ptsX).arg(pdfViewRect.ptsY).arg(
                             pdfViewRect.ptsWidth).arg(pdfViewRect.ptsHeight);
